...
#'@title Print method for Wikisource objects
#'
#'@description print found objects generally.
#'
#'@param x wikisource object from get_item, get_random_item, get_property or get_random_property
#'@param \dots Arguments to be passed to methods
...
print.wikisource <- function(x, ...){
  ...
  if(num.labels>0) {
    ...
    if(num.labels==1) cat("Label:\t\t", lbl, "\n")
      ...
      if(!is.null(x$labels$ko)) lbl <- x$labels$en$value
